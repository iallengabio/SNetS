...
AGS = '_GroomingStatistics.csv'
AEF = '_ExternalFragmentation.csv'
...
MTXU = 'Tx Utilization'
MEFV = 'External Fragmentation (Vertical)'
...
YLegends = ['Circuit Blocking','Bandwidth Blocking','Consumed Energy (J)','Power Consumption (W)','Energy efficiency (b/J)','Weighted Energy efficiency (b/J)','Simplified Profit ($)','Average Transponder Utilization','Request-Circuit Ratio','Deployed BVTs','External Fragmentation']
ArqNames = ['CB.png','BB.png','CE.png','PCE.png','EE.png','WEE.png','PROFIT.png','TXU.png','RRC.png','MBVTU.png','EF.png']
XLegend = 'Network load (Erlangs)'

def extractAllDFS(path,al):
    return [extractDFS(path,ABP,MBP,al),extractDFS(path,ABBP,MBBP,al), extractDFS(path,ACE,MTCE,al), extractDFS(path,ACE,MAPC,al), extractDFSBCR(path,al), extractDFSWEE(path,al), extractDFSProfit(path,al), extractDFS(path,ATRRU,MTXU,al), extractDFS(path,AGS,MRRC,al), extractDFSMBVTU(path,al),extractDFS(path,AEF,MEFV,al)]
...
def plotLines2(path1,path2,loads1=[],loads2=[],sol=[],t1="",t2="", al=0.05):
    ...
    n = n[len(n)-1]
    n = n + '_Line_'
    lDfs1 = extractAllDFS(path1,al)
    lDfs2 = extractAllDFS(path2,al)
...
def plotBars2(path1,path2,loads1=[],loads2=[],sol=[],t1="",t2="", al=0.05):
    n = path1.split('/')
    n = n[len(n)-1]
    n = n + '_Bars_'
    lDfs1 = extractAllDFS(path1,al)
    ...
    for i in range(len(lDfs1)):
        ...
        if(i==1):
            ...
            show = False
        SPlotBar2v(dfs1,dfs2,loads1,loads2,sol,xl,xl,yl,yl,t1,t2,show,arq=a,nc=3)
