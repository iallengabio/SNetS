...
public class LoadBalancedDedicatedPartition implements IntegratedRMLSAAlgorithmInterface {
    ...
    public boolean rsa(Circuit circuit, ControlPlane cp) {
        ...
        for (Route route : candidateRoutes) {
            ...
            if(mod != null){
	            ...
	            int ut = this.numSlotsUsedZone(route, zone, circuit);
     ...
     * @return int
     */
    private int numSlotsUsedZone(Route route, int zone[], Circuit circuit) {
        int res = 0;
        List<int[]> zoneAux = new ArrayList<int[]>();
        zoneAux.add(zone);

        for (Link link : route.getLinkList()) {
            List<int[]> merge = IntersectionFreeSpectrum.merge(link.getFreeSpectrumBands(circuit.getGuardBand()), zoneAux);
