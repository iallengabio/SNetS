...
/**
 * Esta classe representa um algoritmo de alocacao de espectro que utiliza a politica de zonas de espectro dedicadas
 * os algoritmos utilizados sao o FirstFit na zona dedicada
 *
 * @author Felipe
 */

public class DedicatedZoneFirstFit implements SpectrumAssignmentAlgoritm {
    private HashMap<Integer, int[]> zones;

    /**
     * no construtor devem ser passadas por parametro as zonas de divisao do espectro
     * cada zona  descrita por um vetor de 3 posicoes sendo estas respectivamente:
    ...
    public DedicatedZoneFirstFit() {

        List<int[]> zones = null;
        try {
            //zones = ZonesFileReader.readTrafic(Util.projectPath + "/zones");
        } catch (Exception e) {
            System.out.println("nao foi possivel ler o arquivo com a especificacao das zonas!");
    ...
    public boolean assignSpectrum(int numberOfSlots, Circuit request) {
        ...
        int chosen[] = null;
        //primeiro buscar na faixa primaria utilizando o firstFit
        ...
        //System.out.print("zona primaria: " + PrintTests.printFreeSpectrum(lPrimaryZone) );
        ...
        if (chosen == null) return false; //nao encontrou nenhuma faixa contigua e continua disponivel
    ...
    /**
     * passa uma zona como parameetro e eh retornada uma lista de faixas livres correspondente a zona complementar
     *
     * @param zone
     * @return
     */
