...
public class BestSNRDedicatedPathProtection implements SurvivalStrategyInterface {
            	...
            	if(secondaryRoute != null){
					...
					if(infoSecondaryRoute.getCheckRoute() != null){
						...
						if((infoPrimaryRoute.getRoute() != null) && (infoSecondaryRoute.getRoute() != null)){ //rotas com QoTN aceitavel
							...
							double sumSNR = infoPrimaryRoute.getSnr() + infoSecondaryRoute.getSnr();
							boolean QoTforOther = isAdmissibleQoTOther(circuit, infoPrimaryRoute, infoSecondaryRoute, cp);
							
							if((bestSNRAux == 0.0) || (sumSNR > bestSNRAux)){ //menor consumo de energia
								bestSNRAux = sumSNR;
							...
							if(QoTforOther){ //rotas de trabalho e backup com QoTO aceitavel
								if((bestSNR == 0.0) || (sumSNR > bestSNR)){ //menor consumo de energia
									bestSNR = sumSNR;
