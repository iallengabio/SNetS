...
public class DedicatedPathProtection implements SurvivalStrategyInterface {
		...
		if(workRoute != null && backupRoute != null){
			...
			if(groomingAlgorithm.searchCircuitsForGroomingByRoute(rfc, cp, workRoute, backupRoute)){
				...
				// Try to create a new circuit to accommodate the request
		 		...
		 		((SurvivalCircuit)newCircuit).setBackupRoute(backupRoute);
				...
				// Check if it is possible to establish the circuit in the work route
		 		if(tryEstablishNewCircuit(newCircuit, workRoute, cp)){
			 		...
			 		if(tryEstablishNewCircuit(newCircuit, backupRoute, cp)){
			 			...
			 			((SurvivalCircuit)newCircuit).setBackupRoute(backupRoute);
			 			((SurvivalCircuit)newCircuit).setModulationByBackupRoute(backupMod);
			 			((SurvivalCircuit)newCircuit).setSpectrumAssignedByBackupRoute(backupBand);
            	...
            	if(secondaryRoute != null){
					...
					if(infoSecondaryRoute.getCheckRoute() != null){
						...
						if((infoPrimaryRoute.getRoute() != null) && (infoSecondaryRoute.getRoute() != null)){ //rotas com QoTN aceitavel
							...
							if(infosPrimaryAndSecondaryAux == null){
	...
	private InfoRoute checkRoute(Circuit circuit, Route route, ControlPlane cp){
    	...
    	if(chosenMod != null){
			infoRoute = new InfoRoute(route, chosenMod, chosenBand, 0.0, chosenQoTMod, snr);
		}
