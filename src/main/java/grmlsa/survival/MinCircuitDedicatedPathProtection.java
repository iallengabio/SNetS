...
public class MinCircuitDedicatedPathProtection implements SurvivalStrategyInterface {
    ...
    public boolean applyStrategy(Circuit circuit, SurvivalControlPlane cp) {
        ...
        computeBackupRoutes(candidateRoutes, cp);
  		...
  		int minTotalCircuits = 0;
  		
  		InfoRoutes infosPrimaryAndSecondaryAux = null;
  		int minTotalCircuitsAux = 0;
            ...
            if(infoPrimaryRoute.getCheckRoute() != null){
            	...
            	if(secondaryRoute != null){
					...
					if(infoSecondaryRoute.getCheckRoute() != null){
						...
						if((infoPrimaryRoute.getRoute() != null) && (infoSecondaryRoute.getRoute() != null)){ //rotas com QoTN aceitavel
							
							int totalCircuits = getCircuitsByRoute(infoPrimaryRoute.getRoute()) + getCircuitsByRoute(infoSecondaryRoute.getRoute());
							boolean QoTforOther = isAdmissibleQoTOther(circuit, infoPrimaryRoute, infoSecondaryRoute, cp);
							
							if((minTotalCircuitsAux == 0) || (totalCircuits < minTotalCircuitsAux)){ //menor consumo de energia
								minTotalCircuitsAux = totalCircuits;
							...
							if(QoTforOther){ //rotas de trabalho e backup com QoTO aceitavel
								if((minTotalCircuits == 0) || (totalCircuits < minTotalCircuits)){ //menor consumo de energia
									minTotalCircuits = totalCircuits;
    	...
    	for(Modulation mod : avaliableModulations){
            ...
            if (band != null) {
            	...
            	if(flagQoT){ //modulation has acceptable QoT
            		...
            		if((lowestMetricPC == 0.0) || (metricPC < lowestMetricPC)){ //lower power consumption
		                chosenMod = mod;
		                chosenBand = band;
	...
	public int getCircuitsByRoute(Route route){
