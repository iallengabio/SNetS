...
public class BandwidthBlockingProbability extends Measurement{
	...
	public void addNewObservation(boolean success, RequestForConnection request){
		...
		this.generalRequestedBandwidth+=banda;			
		...
		if(i==null) i=0.0;
		this.requestedBandwidthPerPair.put(request.getPair().getSource().getName() + SEP +request.getPair().getDestination().getName(), i+banda);			
		
		// Increment requests generated by bandwidth
		i = this.requestedBandwidthPerBW.get(request.getRequiredBandwidth());
		if(i==null) i=0.0;
		this.requestedBandwidthPerBW.put(request.getRequiredBandwidth(), i+banda);			
		...
		if(!success){
			...
			this.generalBandwidthBlockingProbability+=banda;
			...
			if(i==null) i=0.0;
			this.bandwidthBlockedPerPair.put(request.getPair().getSource().getName() + SEP +request.getPair().getDestination().getName(), i+banda);			
			
			// Increment blocked requests per bandwidth
			i = this.bandwidthBlockedPerBW.get(request.getRequiredBandwidth());
			if(i==null) i=0.0;
			this.bandwidthBlockedPerBW.put(request.getRequiredBandwidth(), i+banda);			
