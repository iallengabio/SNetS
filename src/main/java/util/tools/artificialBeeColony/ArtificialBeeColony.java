...
public class ArtificialBeeColony {
    ...
    public void sendToWork(FoodSource currentBee, FoodSource neighborBee) {
        ...
        double rid = (rand.nextDouble() - 0.5) * 2; //It is a random real number within the ranger [-1, 1]
        newValue = currentValue + rid * (currentValue - neighborValue);
	...
	public void calculateProbabilities() {
        ...
        for(int i = 1; i < foodNumber; i++) {
            ...
            if(thisFood.getFitness() >= 0.0) {
                sum += 1.0 / (1.0 + thisFood.getFitness());
            	
            } else {
            	sum += 1.0 + Math.abs(thisFood.getFitness());
            }
        }
        
        for(int j = 0; j < foodNumber; j++) {
            thisFood = foodSources.get(j);
            thisFood.setSelectionProbability(thisFood.getFitness() / sum);
        }
    }
